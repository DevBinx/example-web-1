pipeline {
    ...
    environment {
        ...
        DOCKER_IMAGE = 'nginxstore/webapp-example-1:latest'
        DOCKER_CONTAINER_NAME = 'webapp1'
        DOCKERFILE_PATH = './Dockerfile'
        DIST_DIR = '/var/jenkins_home/workspace/webapp1-pipeline/example-web-1/dist'
    ...
    stages {
        stage('Checkout') {
            steps {
                ...
                git branch: 'main', url: 'https://github.com/DevBinx/example-web-1.git'
            }
        }
        ...
        stage('Docker Build') {
            steps {
                script {
                    ...
                    # 새로운 컨테이너 실행
                    docker run -d -p 8011:80 --name ${DOCKER_CONTAINER_NAME} ${DOCKER_IMAGE}
